...
SetTitle("SecondaryGunz - Gift Complete Set");

if($_SESSION[AID] == "")
{
    SetURL("index.php?do=giftset&setid={$_GET[setid]}");
...
if(!isset($_POST[SetID]))
{
    ...
    {
        SetMessage("Message from Shop", array("Informacion de set Incorrecta"));
...
if(isset($_POST[SetID]))
{
    ...
    $recid  = clean($_POST[recipient]);
        ...
        break;
        default:
        SetMessage("Message from Shop", array("The amount of rent days was edited"));
        header("Location: index.php?do=shopsets");
        die();
    }
    ...
    if(strlen($recid) == 0)
    {
        SetMessage("Error from Shop", array("Please enter the UserID from the beneficiary"));
        header("Location: index.php?do=giftset&setid=$setid");
        die();
    }
    ...
    {
        SetMessage("Error from Shop", array("The beneficiary does not exist."));
        header("Location: index.php?do=giftset&setid=$setid");
        die();
    }
    ...
    if($afterbalance < 0)
    {
        SetMessage("Error from Shop", array("Item gifted to $recid"));
        header("Location: index.php?do=giftset&setid=$setid");
        die();
        ...
        if( $ires->HeadItemID != 0 )
        {
            mssql_query_logged("INSERT INTO AccountItem(AID, ItemID, RentDate, RentHourPeriod, Cnt)VALUES" .
                        "({$rres->AID}, {$ires->HeadItemID}, GETDATE(), $renthourperiod, 1)");
        }
        //Chest
        if( $ires->ChestItemID != 0 )
        {
            mssql_query_logged("INSERT INTO AccountItem(AID, ItemID, RentDate, RentHourPeriod, Cnt)VALUES" .
                        "({$rres->AID}, {$ires->ChestItemID}, GETDATE(), $renthourperiod, 1)");
        }
        //Hand
        if( $ires->HandItemID != 0 )
        {
            mssql_query_logged("INSERT INTO AccountItem(AID, ItemID, RentDate, RentHourPeriod, Cnt)VALUES" .
                        "({$rres->AID}, {$ires->HandItemID}, GETDATE(), $renthourperiod, 1)");
        }
        //Leg
        if ( $ires->LegItemID != 0 )
        {
            mssql_query_logged("INSERT INTO AccountItem(AID, ItemID, RentDate, RentHourPeriod, Cnt)VALUES" .
                        "({$rres->AID}, {$ires->LegItemID}, GETDATE(), $renthourperiod, 1)");
        }
        //Feet
        if ( $ires->FeetItemID != 0 )
        {
            mssql_query_logged("INSERT INTO AccountItem(AID, ItemID, RentDate, RentHourPeriod, Cnt)VALUES" .
                        "({$rres->AID}, {$ires->FeetItemID}, GETDATE(), $renthourperiod, 1)");
        }

        SetMessage("Item Shop", array("You have gifted the set to $recid successfully."));
        header("Location: index.php?do=shopsets");
        die();
					...
					<tr>
						...
						<div align="center">
							<table border="0" style="background-position: center top; border-collapse: collapse; background-image:url('images/content_bg.jpg'); background-repeat:repeat-y" width="603">
								<tr>
									<td style="background-image: url('images/content_title_shop_gift.jpg'); background-repeat: no-repeat; background-position: center top" height="25" width="601" colspan="3">&nbsp;</td>
								</tr>
								<tr>
									...
									<div align="center">
										<table border="1" style="border-collapse: collapse; border: 1px solid #4A4648" width="100%" bordercolor="#4A4648">
											<tr>
												<td>
												<div align="center">
													<form method="POST" action="index.php?do=giftset" name="frmBuy"><table border="0" style="border-collapse: collapse" width="579" height="100%">
														<tr>
															<td width="11">&nbsp;</td>
															<td width="104">&nbsp;</td>
																	...
																	<tr>
																		...
																		<div align="center">
																			<table border="0" style="border-collapse: collapse" width="419" height="100%">
																				<tr>
																					<td width="216">Gift to:<input type="text" name="recipient" size="10" class="textLogin"></td>
								...
								<tr>
									...
									<img border="0" src="images/btn_giftset_off.jpg" width="79" height="23" id="img1764" onMouseOut="FP_swapImgRestore()" onMouseOver="FP_swapImg(1,1,/*id*/'img1764',/*url*/'images/btn_giftset_on.jpg')"></a>
